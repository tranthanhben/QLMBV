...
import {changeDTI, datetime, reveserChangeDTI, setValue, getMonth} from '../../meta';
...
export class NXMonth extends Component {
  ...
  componentWillMount(){
    this.props.loadNX(this.state.options || {period: 'month'});
  }
              ...
              <div className="dataTables_length" id="example_length" style={{"display": "inline-flex"}}>
                <div className="row">
                  <div className="col-md-6">
                    <div className='form-group' key="batdau">
                      <label>
                        <span>
                          {'Từ ngày'}
                        </span>
                        <input type="month" max={options.end} className="form-control" value={options.start || ''} onChange={::this.changeDate} data-addr="start"/>
                      </label>
                    </div>
                  </div>
                  <div className="col-md-6">
                    <div className='form-group' key="ketthuc">
                      <label>
                        <span>
                          {'Đến ngày'}
                        </span>
                        <input type="month" min={options.start} className="form-control" value={options.end || ''} onChange={::this.changeDate} data-addr="end"/>
                      </label>
                    </div>
                  </div>
                </div>
